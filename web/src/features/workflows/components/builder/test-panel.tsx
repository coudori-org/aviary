...
const STATUS_CONFIG: Record<string, { icon: React.ReactNode; color: string; label: string }> = {
  running: { icon: <Loader2 size={11} className="animate-spin" />, color: "text-accent", label: "Running" },
  completed: { icon: <Check size={11} strokeWidth={3} />, color: "text-status-live", label: "Done" },
  failed: { icon: <X size={11} strokeWidth={3} />, color: "text-status-error", label: "Failed" },
  skipped: { icon: <CircleDot size={11} />, color: "text-fg-muted", label: "Skipped" },
};
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1 text-[10px] font-medium text-fg-muted uppercase tracking-wider hover:text-fg-muted transition-colors"
      >
...
function DataPreview({ label, data, defaultOpen = false }: { label: string; data: unknown; defaultOpen?: boolean }) {
  ...
  return (
    <Collapsible label={label} defaultOpen={defaultOpen}>
      <pre className="text-[11px] text-fg-muted bg-hover rounded-md px-2 py-1.5 overflow-x-auto whitespace-pre-wrap break-words font-mono">
        {text}
      </pre>
    </Collapsible>
  );
...
function NodeCard({
  ...
  return (
    <div className={cn(
      "rounded-lg border border-border-subtle bg-raised overflow-hidden",
      status === "running" && "border-accent-border",
    )}>
      ...
      <div className="px-3 py-2 space-y-2">
        ...
        {status === "skipped" && <p className="text-[11px] text-fg-muted">Skipped (condition branch)</p>}
        {status === "failed" && data.error && (
          <p className="text-[11px] text-status-error">{data.error}</p>
        )}
      </div>
    </div>
  );
...
function RunFooter({
  ...
  return (
    <div className="shrink-0 border-t border-border-subtle flex flex-col items-center gap-1.5 px-3 py-2">
      <p className={cn("text-[11px]", terminal === "success" ? "text-status-live" : "text-status-error")}>{label}</p>
      {showResume && (
        <button
          type="button"
          onClick={run.resume}
          className="flex items-center gap-1.5 rounded-md border border-accent-border bg-accent-soft px-2.5 py-1 text-[11px] font-medium text-accent hover:bg-accent-soft transition-colors"
      ...
      <div className="flex items-end gap-2">
        <textarea
          ...
          className={cn(
            ...
            "border border-border-default placeholder:text-fg-muted",
            "focus:outline-none focus:border-accent-border focus:ring-1 focus:ring-accent",
            "disabled:opacity-40",
          )}
        />
        {run.isRunning ? (
          <button
            type="button"
            onClick={run.cancel}
            className="shrink-0 flex items-center justify-center h-9 w-9 rounded-md bg-status-error-soft text-status-error hover:bg-status-error-soft transition-colors"
            title="Cancel run"
          >
          ...
          <button
            ...
            className="shrink-0 flex items-center justify-center h-9 w-9 rounded-md bg-accent-soft text-accent hover:bg-accent-soft transition-colors disabled:opacity-30"
            title="Start run"
          >
...
function RunningStatusBar({ run }: { run: ReturnType<typeof useWorkflowRun> }) {
  ...
  return (
    <div className="shrink-0 flex items-center justify-between gap-2 border-b border-border-subtle px-3 py-2 bg-accent-soft">
      ...
      <button
        type="button"
        onClick={run.cancel}
        className="flex items-center gap-1 rounded-md bg-status-error-soft px-2 py-1 text-[11px] font-medium text-status-error hover:bg-status-error-soft transition-colors"
      >
...
export function TestPanel({ run }: TestPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {isAgentStepFocus ? (
        // Embed the same chat renderer the agent page uses (header / input /
        // workspace stripped). `live` gates the WS subscription to running
        // steps only — a terminal-state transcript doesn't need a live
        // socket.
        <>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
          {focusData ? (
            ...
            <p className="text-center text-[12px] text-fg-muted py-8">
              {isTriggerFocus
      ...
      {isTriggerFocus && (
        canTrigger ? (
          ...
          <div className="shrink-0 border-t border-border-subtle px-3 py-3 type-caption text-fg-muted text-center">
